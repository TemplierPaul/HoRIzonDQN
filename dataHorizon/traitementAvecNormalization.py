...
for nom in os.listdir('.'):
    if nom.endswith('.csv'):
        ...
        for ligne in mydata.iterrows():
            ...
            #print (info)
            mydata.at[ligne[0], 'remaining_time'] =(float(mydata.at[ligne[0], 'remaining_time'])-300)/ 300
            if int(info[1]) > auto:
                auto = int(info[1])
            ...
            mydata.at[ligne[0], 'robot_x'] = (float(mydata.at[ligne[0], 'robot_x']) -0.0)/ 16
            mydata.at[ligne[0], 'robot_y'] = (float(mydata.at[ligne[0], 'robot_y'])-0.0) / 16
            mydata.at[ligne[0], 'robot_angle'] = (float(mydata.at[ligne[0], 'robot_angle']) - 3.15) / 3.15
            mydata.at[ligne[0], 'battery_level'] = (float(mydata.at[ligne[0], 'battery_level']) -50)/100
            mydata.at[ligne[0], 'temperature'] = (float(mydata.at[ligne[0], 'temperature']) - 50) / 50
            mydata.at[ligne[0], 'robot_tank_water_level'] = (float(mydata.at[ligne[0], 'robot_tank_water_level']) - 25) / 25
            mydata.at[ligne[0], 'ground_tank_water_level'] = (float(mydata.at[ligne[0], 'ground_tank_water_level']) - 25) / 25
            trees = info[6].split('-')
            #            print (trees)
            for i in range(0, 9):
                if trees[i] == 'true':
